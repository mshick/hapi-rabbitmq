...
const addWorker = function ({connection, queue: queueName, worker, options}, plugin) {
  ...
  return channelCreated
    .then(ch => {
      ...
      if (retryOptions && retryOptions.queueSuffix) {
        const {queueSuffix, maxLength} = retryOptions;
      ...
      if (doneOptions && doneOptions.queueSuffix) {
        const {queueSuffix, maxLength} = doneOptions;
        const dQueueName = `${queueName}${queueSuffix || '_done'}`;
