...
const keepConnectionAlive = function (openConnection, options, plugin) {
  const {server, name: pluginName} = plugin;
  const connectionName = openConnection._connectName;
  const onConnectionError = () => {
    server.log(['info', pluginName], `connection error, attempting to reconnect`);
...
const createConnection = function (localOptions, plugin) {
  const {options: pluginOptions, state: pluginState, name: pluginName, server} = plugin;
  ...
  return new Promise((resolve, reject) => {
    operation.attempt(number => {
      server.log(['info', pluginName], `connection attempt ${number}`);
      amqp.connect(connectUrl, connectionOptions.socket)
        .then(connection => {
          ...
          server.log(['info', pluginName], 'connection opened');
          resolve(connection);
        })
        .catch(err => {
