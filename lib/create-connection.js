...
const keepConnectionAlive = function (openConnection, options, plugin) {
  ...
  const onConnectionError = () => {
    plugin.log(['info', plugin.name], `connection error, attempting to reconnect`);
...
const createConnection = function (localOptions, plugin) {
  ...
  return new Promise((resolve, reject) => {
    operation.attempt(number => {
      plugin.log(['info', plugin.name], `connection attempt ${number}`);
      amqp.connect(connectUrl, connectionOptions.socket)
        .then(connection => {
          ...
          plugin.log(['info', plugin.name], 'connection opened');
          resolve(connection);
        })
        .catch(err => {
